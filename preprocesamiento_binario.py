...
===============================================================================
Este script lee directo de data/raw_download/ -- la carpeta donde
descargar_pannuke.py deja el dataset (train/images, train/masks,
validate/images, validate/masks). No copia nada a otra ubicacion primero:
trabaja sobre esos mismos archivos, incluyendo el split de test que crea
...
DIFERENCIA CON EL PANNUKE OFICIAL:
  ...
  El mirror que se uso aqui (llwlabs/pannuke) ya viene como PNG con
  mascaras BINARIAS (0 = fondo, 255 = nucleo, sin distincion de tipo) y
  solo 2 splits (train/validate, sin test). Por eso este script:
...
def split_validate_into_val_test(
    ...
    validate/ -- a proposito se usa MOVE y no COPY: si se copiara, los mismos
    archivos quedarian en validate/ y en test/ a la vez, lo cual mezclaria
    los dos splits (data leakage entre validacion y prueba). Si en algun
    momento quieres rehacer este split con otra proporcion, vuelve a extraer
    'validate' desde data/raw_download/ (la salida de descargar_pannuke.py),
    que queda intacta y sirve como respaldo. Es idempotente: si test/ ya
    existe y tiene archivos, no hace nada.
    """
    ...
    print(f"[OK] 'test' listo con {len(test_filenames)} pares; 'validate' quedo con {len(filenames) - len(test_filenames)} (sin solaparse).")
